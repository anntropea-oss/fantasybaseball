...
const dashboardData = {
  ...
  latest: {
    ...
    modelStatus: latest.modelStatus || null,
    protectedInjuryReviews: latestContext.protectedInjuryReviews || [],
    dropDiagnostics: latestContext.dropDiagnostics || null,
  },
};

const initialDataJson = JSON.stringify(dashboardData).replaceAll("<", "\\u003c");
const dataFileName = "dashboard-data.json";
const clientFileName = "dashboard.js";
...
<head>
  ...
  <link rel="preload" href="./${dataFileName}" as="fetch" />
  <style>
    ...
    .pill { display: inline-block; border: 1px solid var(--border); border-radius: 999px; padding: 2px 8px; margin: 2px 4px 2px 0; font-size: 12px; color: var(--muted); background: #f8fafc; }
...
<body>
  <div class="wrap">
    ...
    <div class="meta" id="dashboard-meta">Built from <code>${escapeHtml(dataSource)}</code> • Window: ${escapeHtml(dates[0])} → ${escapeHtml(dates.at(-1))} • Mode: ${escapeHtml(args.mode)} • Updated: ${escapeHtml(generatedAt)} • Auto-refreshing</div>
    <div id="dashboard-root"><div class="card">Loading dashboard data...</div></div>
  </div>
  <script>window.__DASHBOARD_DATA__ = ${initialDataJson};</script>
...
fs.writeFileSync(path.join(path.dirname(args.out), dataFileName), JSON.stringify(dashboardData, null, 2));
fs.writeFileSync(path.join(path.dirname(args.out), clientFileName), clientJs);
