...
## [2026-06-04 10:51] Add Honest Regression Diagnostics And Standings Surface
- Problem: The dashboard's regression card implied predictive value from a one-feature start-adherence scatterplot even though experiments showed direct team-day roto-point regression has near-zero or negative walk-forward R2.
- Root Cause: The dashboard only modeled inferred recommended starts against total roto-point movement, while snapshots discarded the full league category standings surface needed to convert category-value forecasts into point/rank threshold simulations.
- Solution: Added full league category standings capture to snapshots and SQLite `league_category_standings`; replaced the dashboard regression section with model diagnostics that report the toy start R2, category point walk-forward R2, category value walk-forward R2, and the recommended category-value forecast plus full-standings simulation path. Updated E2E coverage and documentation.
- Files Changed: `/Users/atropea/coding/fantasy baseball/fantasy/cli.js`, `/Users/atropea/coding/fantasy baseball/fantasy/scripts/dashboard.mjs`, `/Users/atropea/coding/fantasy baseball/fantasy/docs/index.html`, `/Users/atropea/coding/fantasy baseball/fantasy/docs/dashboard-data.json`, `/Users/atropea/coding/fantasy baseball/fantasy/docs/dashboard.js`, `/Users/atropea/coding/fantasy baseball/fantasy/tests/e2e/run-e2e.mjs`, `/Users/atropea/coding/fantasy baseball/fantasy/README.md`, `/Users/atropea/coding/fantasy baseball/fantasy/SOLUTIONS.md`
- Status: Resolved
- Verification: `node --check cli.js`, `node --check scripts/dashboard.mjs`, and `caffeinate -dimsu node --test tests/e2e/run-e2e.mjs` passed; live `node cli.js recommend` wrote a snapshot with 10 league-standing categories and SQLite stored 130 `league_category_standings` rows; `node cli.js dashboard --publish` generated diagnostics with toy starts R2 0.026, category point R2 0.011, and category value R2 0.111.

## [2026-06-05 10:29] Add Actionability-Weighted Champion Gate
- Problem: The champion/challenger benchmark could still promote or reject target models based mostly on theoretical category quality, without fully accounting for whether recommendations were executable or likely to translate into league-rank gains.
- Root Cause: `scripts/model-benchmark-deep.mjs` scored raw and rank-aware target gains, while `cli.js` promotion sorted by rank-aware deltas. Safe-drop availability, blocked add candidates, start risk, start execution, direct pressure on the team above us, and add/drop-vs-start actionability were not first-class promotion criteria.
- Solution: Added actionability diagnostics and actionability-weighted gain to the benchmark, including safe-drop bottleneck rate, blocked adds, start ratio-risk penalty, start execution rate, and direct next-team category gain. Updated CLI challenger sorting, champion qualification, and model-status output to use the actionability-weighted objective with rank-aware/raw safety gates and a non-negative direct-next-team gate. Updated README benchmark documentation.
- Files Changed: `/Users/atropea/coding/fantasy baseball/fantasy/scripts/model-benchmark-deep.mjs`, `/Users/atropea/coding/fantasy baseball/fantasy/cli.js`, `/Users/atropea/coding/fantasy baseball/fantasy/README.md`, `/Users/atropea/coding/fantasy baseball/fantasy/SOLUTIONS.md`
- Status: Resolved
